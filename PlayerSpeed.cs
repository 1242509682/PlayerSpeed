...
public class PlayerSpeed : TerrariaPlugin
{
    ...
    private void OnPlayerUpdate(object? sender, GetDataHandlers.PlayerUpdateEventArgs e)
    {
        ...
        {
            AddSpeed(plr, tplr, data, Config.CoolTime, Config.Count, Config.Speed, Config.Height);
        }
        else
        {
    ...
    private static void AddSpeed(TSPlayer plr, Player tplr, Database.PlayerData? data,int CoolTime,int Count ,float Speed, float Height)
    {
        ...
        if (!data.Enabled && LastCool >= CoolTime)
        {
            ...
            if (Config.Mess)
            {
                ...
                if (Config.Dash)
                {
                    plr.SendMessage("使用克盾类饰品[c/47DCBD:冲刺]可冲更远", 244, 255, 150);
                }
        ...
        {
            tplr.velocity.X = Speed * Config.Multiple * tplr.direction;
        ...
        if (Config.Jump && data.Enabled && armor && CheckJupm(tplr, data, CLeft, CRight))
        {
            ...
            if (CLeft && tplr.controlDown)
            {
                tplr.velocity.Y = Height / Config.Multiple2 / tplr.direction;
            }
    ...
    internal static BossData GetMaxSpeed(List<BossData> bossList)
    {
        ...
        // 初始化最大值
        BossData maxBoss = bossList.Where(b => b.Enabled).FirstOrDefault()!;
        if (maxBoss == null) return null; // 没有已启用的Boss
